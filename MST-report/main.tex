...
% Feedback Iterations
\draw[dashedarrow] (gate1.west) -- ++(-1.6,0)
  node[pos=0.5, above=1pt, font=\scriptsize\bfseries, text=red!75!black, fill=white, inner sep=2pt]{Quality Gate Fail}
  |- (sp2.west)
  node[pos=0.85, above=1pt, font=\scriptsize\bfseries, text=red!75!black, fill=white, inner sep=2pt]{Refactor Sprint};
\draw[dashedarrow] (eval.east) -- ++(1.6,0)
  |- (arch.east)
  node[pos=0.25, right=2pt, font=\scriptsize\bfseries, text=red!75!black, fill=white, inner sep=2pt]{Sprint Retrospective};
...
\node[state] (s1) at (0, 0) {\textbf{Phase 1: Baseline}\\[3pt]\footnotesize 3 Nodes Healthy\\\footnotesize (:8001--:8003)\\[2pt]\footnotesize 33.3\% Traffic Each};
\node[state] (s2) at (6.8, 0) {\textbf{Phase 2: Chaos Injection}\\[3pt]\footnotesize Backend 2 (:8002) Killed\\[2pt]\footnotesize In-Flight Socket Catch};
\node[state] (s3) at (13.6, 0) {\textbf{Phase 3: Self-Healing}\\[3pt]\footnotesize Backend 2 Restored\\[2pt]\footnotesize Health Ping Returns 200};

\draw[critarrow] (s1.east) -- node[above=3pt, font=\scriptsize\bfseries, text=red!80!black]{Power Switch OFF} node[below=3pt, font=\scriptsize]{Reactive Reroute (0 drops)} (s2.west);
\draw[recovarrow] (s2.east) -- node[above=3pt, font=\scriptsize\bfseries, text=teal!80!black]{Power Switch ON} node[below=3pt, font=\scriptsize]{Auto-Reintegration $\le$ 3.0s} (s3.west);
